class Calculation {
  ...
  String getString() {
    String str = ""; a.forEach((String el) {str+=el;}); return str;
  }

  void deleteOne() { if(a.length > 0) {
    if(a.last.length > 1) {
  ...
  double getResult() {
    ...
    for(int i = 0; i < a.length; i++) { if(a[i] == "x") {
      ...
      a[i-1] = "${double.parse(a[i-1]) * double.parse(a[i+1])}"; a.removeAt(i);
      a.removeAt(i);
      i--;
    } else if(a[i] == "÷") {
    ...
    if(a.length != 1) throw Error(); return double.parse(a[0]);
  }
